class MonstersController < ApplicationController
  ...
  def index
    ...
    unless params[:sort].blank?
      case params[:sort]
      when "name"
      @monsters = @monsters.order("name asc")
      when "power"
      @monsters = @monsters.order("power asc")
      when "type"
        @monsters = @monsters.sort_by do |monster| Monster::TYPES.index(monster.monster_type) end
      end
    end
  end
  ...
  private
    ...
    def monster_params
      params.require(:monster).permit(:name, :power, :monster_type, :user_id)
    end
end
